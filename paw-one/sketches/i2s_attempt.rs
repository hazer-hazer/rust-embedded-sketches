...
async fn main(_spawner: Spawner) {
    ...
    loop {
        // let mut next_half_buf = [0; BUFFER_SIZE];
        // let mut last_val = 0;
        // for (i, s) in next_half_buf.iter_mut().enumerate() {
        //     if i % 2 == 0 {
        //         *s = (sound.next().unwrap() * i32::MAX as f32) as i32 as u32;
        //         last_val = *s;
        //     } else {
        //         *s = last_val;
        //     }
        ...
        // // info!("Send buffer {:?}", next_half_buf);

        i2s_spi.blocking_write(&sine).unwrap();
    }

    // loop {
    //     info!("Generate next audio buffer");
    //     let mut next_half_buf = [0; BUFFER_SIZE];
    //     for s in next_half_buf.iter_mut() {
    //         *s = (sound.next().unwrap() * i32::MAX as f32) as i32 as u32;
    //     }

    //     info!("Send buffer {:?}", next_half_buf);

    //     sai.write(&next_half_buf).await.unwrap();
    // }
